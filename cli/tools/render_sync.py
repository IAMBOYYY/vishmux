#!/usr/bin/env python3
"""
VISHMUX Render Sync – pushes the CLI's active AI provider/key to Supabase's
...
async def sync_ai_config_to_render(config) -> None:
    """
    Push the active provider's key/base_url/model to Supabase's ai_config
    table (upsert, single row id=1). Always fails safe — never raises,
    silently no-ops if Supabase isn't configured, no provider is active,
    or the active provider isn't OpenAI-compatible.
    """
    try:
        ...
        print(f"[render_sync] Synced '{provider_name}' / {model} to Render.")
